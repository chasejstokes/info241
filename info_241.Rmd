...
analysis <- analysis %>%
  ...
  select(technical_scale, leadership_scale, likeable_scale, learner_scale, education_scale, experience_scale, 
         colnames(analysis)[! colnames(analysis) %in% c(technical_scale_cols, leadership_scale_cols,
                                                        ...
                                                        experience_scale_cols, "stimuli_number", "source")])
...
measures_model <- d[, lm(data = analysis, rating ~ applicant_gender * applicant_education + risk + 
                           factor(gender, levels = c("Man", "Woman"))+
                           factor(company_size) +
                           technical_scale +
                           leadership_scale +
...
# test for heteroscedasticity to determine if use of robust standard errors is justified
bptest(measures_model)

measures_model$vcovHC_ <- vcovHC(measures_model)
coefs_measures <- coeftest(measures_model, vcov. = measures_model$vcovHC_)
coefs_measures
ci_measures <- coefci(measures_model, vcov. = measures_model$vcovHC_)
ci_measures
