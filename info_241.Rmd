...
analysis <- analysis %>%
  mutate(technical_scale = invoke(coalesce, across(all_of(technical_scale_cols))),
         leadership_scale = invoke(coalesce, across(all_of(leadership_scale_cols))),
         likeable_scale = invoke(coalesce, across(all_of(likeable_scale_cols))),
         learner_scale = invoke(coalesce, across(all_of(learner_scale_cols))),
         education_scale = invoke(coalesce, across(all_of(education_scale_cols))),
         experience_scale = invoke(coalesce, across(all_of(experience_scale_cols)))) %>% 
...
gender_condition_dist <- ggplot(pt_info, aes(x = as.factor(gender)))+
  ...
  labs(title = "Distribution of Participant Gender and Condition",
       subtitle = "Conditions were relatively even across gender.",
       x = "Reported Gender",
       y = "Count")
gender_condition_dist
...
## linear model
condition_model <- d[, lm(rating ~ Condition)]

# test for heteroscedasticity to determine if use of robust standard errors is justified
par(mfrow = c(2, 2))
plot(condition_model)
bptest(condition_model)

condition_model$vcovHC_ <- vcovHC(condition_model)
coefs_conditions <- coeftest(condition_model, vcov. = condition_model$vcovHC_)
coefs_conditions
ci_conditions <- coefci(condition_model, vcov. = condition_model$vcovHC_)
ci_conditions
...
applicant_model <- d[, lm(rating ~ applicant_gender * applicant_education)]

# test for heteroscedasticity to determine if use of robust standard errors is justified
par(mfrow = c(2, 2))
plot(applicant_model)
bptest(applicant_model)

applicant_model$vcovHC_ <- vcovHC(applicant_model)
coefs_applicants <- coeftest(applicant_model, vcov. = applicant_model$vcovHC_)
coefs_applicants
ci_applicants <- coefci(applicant_model, vcov. = applicant_model$vcovHC_)
ci_applicants
...
risk_model <- d[, lm(data = analysis, rating ~ applicant_gender * applicant_education + risk)]

# test for heteroscedasticity to determine if use of robust standard errors is justified
bptest(risk_model)

risk_model$vcovHC_ <- vcovHC(risk_model)
coefs_risk <- coeftest(risk_model, vcov. = risk_model$vcovHC_)
coefs_risk
ci_risk <- coefci(risk_model, vcov. = risk_model$vcovHC_)
ci_risk
...
gender_model <- d[, lm(data = analysis, rating ~ applicant_gender * applicant_education + risk + 
                        factor(gender, levels = c("Man", "Woman")))]

# test for heteroscedasticity to determine if use of robust standard errors is justified
bptest(gender_model)

gender_model$vcovHC_ <- vcovHC(gender_model)
coefs_gender <- coeftest(gender_model, vcov. = gender_model$vcovHC_)
coefs_gender
ci_gender <- coefci(gender_model, vcov. = gender_model$vcovHC_)
ci_gender
...
company_model <- d[, lm(data = analysis, rating ~ applicant_gender * applicant_education + risk + 
                        factor(gender, levels = c("Man", "Woman"))+
                        factor(company_size))]

# test for heteroscedasticity to determine if use of robust standard errors is justified
bptest(company_model)

company_model$vcovHC_ <- vcovHC(company_model)
coefs_company <- coeftest(company_model, vcov. = company_model$vcovHC_)
coefs_company
ci_company <- coefci(company_model, vcov. = company_model$vcovHC_)
ci_company
...
stargazer(
  ...
  company_model, 
  type = 'text',
  se = list(sqrt(diag(applicant_model$vcovHC_)),
            sqrt(diag(risk_model$vcovHC_)),
            sqrt(diag(gender_model$vcovHC_)),
            sqrt(diag(company_model$vcovHC_))
  )
)
